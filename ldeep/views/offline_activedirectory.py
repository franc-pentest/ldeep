...
from ldeep.views.activedirectory import ALL
from ldeep.views.constants import WELL_KNOWN_SIDs


class OfflineActiveDirectoryView(ActiveDirectoryView):
	...
	ANR = lambda s, u: {"files": ["users_all", "groups", "machines"], "filter": lambda x: x["sAMAccountName"] == u}
	GROUP_DN_FILTER = lambda s, g: {"fmt": "json", "files": ["groups"], "filter": lambda x: x["sAMAccountName"] == g}
	USERS_IN_GROUP_FILTER = lambda s, p, g: {
		"files": ["users_all", "groups"],
		"filter": lambda x: ("primaryGroupID" in x and p == x["primaryGroupID"]) or ("memberOf" in x and g in x["memberOf"])
	}
	USER_IN_GROUPS_FILTER = lambda s, u: {
		"fmt": "json",
		"files": ["users_all", "machines"],
		"filter": lambda x: x["sAMAccountName"] == u
	}
	DISTINGUISHED_NAME = lambda s, n: {
		"fmt": "json",
		"files": ["users_all", "groups", "machines"],
		"filter": lambda x: x["distinguishedName"] == n
	}
	# Not implemented:
	...
	def query(self, cachefilter, attributes=ALL, base=None, scope=None, **filter_args):
		...
		for fil in ldapfilter["files"]:
			...
			with open(path.join(self.path, filename)) as fp:
				if fmt == "json":
					json = json_load(fp)
					if "filter" in ldapfilter:
						for record in json:
							if ldapfilter["filter"](record):
								data.append(record)
					else:
						data += json								
				else:
