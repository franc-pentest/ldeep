...
from ldap3 import Connection, SASL, KERBEROS, NTLM, Server
import socket, ssl
...
def checkLDAPSigning(server, userDN, password, kerberosAuth):
	try:
		if kerberosAuth:
			...
			conn = Connection(server, userDN, password, authentication = NTLM, auto_bind = False)
	except:
		return None
...
def checkEPAPolicy(server, userDN, password):
	try:
		...
		conn = Connection(server, userDN, password, authentication = NTLM, channel_binding = 'TLS_CHANNEL_BINDING', auto_bind = False)
	except Exception as e:
		return None
	originalSTDOUT = sys.stdout
	originalSTDERR = sys.stderr
...
def checkProtections(target, username, password, ntlm, domain, kerberosAuth):
	...
	if not LDAPSCompleteHandshake(target):
		...
		policyEPA = None
		ldapsChannelBindingAlways = checkEPA(serverLDAPS, user_dn, password, kerberosAuth)
		if ldapsChannelBindingAlways == None:
			...
			if ldapsChannelBindingAlways == True:
				...
				if kerberosAuth:
					print("[-] Channel Binding (EPA) policy cannot be verified with Kerberos authentication")
				else:
					ldapsChannelBindingWhenSupported = checkEPAPolicy(serverLDAPS, user_dn, password)
...
def main():
	...
	protections = sub.add_parser("protections", description="LDAP Signing & LDAPS Channel Binding verification")
	...
	protections.add_argument("-H", "--ntlm", help="NTLM hashes, format is LMHASH:NTHASH")
	protections.add_argument(
		"-k",
		"--kerberos",
		action="store_true",
	...
	if cache:
		...
		checkProtections(args.ldapserver, args.username, args.password, args.ntlm, args.domain, args.kerberos)
		exit()
